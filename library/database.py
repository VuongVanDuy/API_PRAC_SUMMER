...
class DbManagerUser:
    ...
    def insert_user(self, username, password, link_icon, token):
        try:
            ...
            self.cursor.execute('''INSERT INTO user(username, password, link_icon, token) 
                                VALUES (?, ?, ?, ?)''', (username, hash_password, link_icon, token))
            self.conn.commit()
            return True
        except Exception as e:
            print(e)
    ...
    def get_token_user(self, name_user):
        try:
            self.cursor.execute("SELECT token FROM user WHERE username = ?", (name_user,))
            return self.cursor.fetchone()[0]
        except Exception as e:
            return None
